    ...
    const getPreview = async () => {
      ...
      try {
        ...
        const response = await fetch(
          "http://localhost:3000/api/bookings/preview",
          {
            method: "POST",
            headers: {
    ...
    <div className="booking-page-root">
      ...
      <div className="title-section">
        ...
        <p className="sub-title">
          เลือกบริการที่ต้องการและระบุวันเวลาเพื่อเข้ารับบริการ
        </p>
      </div>
      ...
      <div className="step-container">
        ...
        {mainServices.map((s) => (
          <div
            key={s.id}
            className={`service-item ${selectedService === s.id ? "selected" : ""}`}
            onClick={() => setSelectedService(s.id)}
          >
            <div className="radio-circle">
              {selectedService === s.id && <div className="radio-inner" />}
            </div>
        ...
        <span className="service-group-label" style={{ marginTop: "30px" }}>
          บริการเสริม
        </span>
        {addons.map((a) => (
          <div
            key={a.id}
            className={`service-item ${selectedAddons.includes(a.id) ? "selected" : ""}`}
            onClick={() => toggleAddon(a.id)}
          >
            <div className="radio-circle" style={{ borderRadius: "4px" }}>
              {selectedAddons.includes(a.id) && (
                <Check size={14} color="#d40000" strokeWidth={4} />
              )}
            </div>
            <div className="service-info">
      ...
      {selectedService && (
        ...
        >
          {/* 🎯 เพิ่มส่วนนี้: แจ้งเตือนถ้ารถมีค่าบริการเพิ่ม (เช่น รถไซส์ใหญ่) */}
          {previewData?.vehicle_extra > 0 && (
            <div
              style={{
                display: "flex",
            ...
            >
              <span>* ค่าบริการเพิ่มเติม (ขนาดรถ)</span>
              <span>+{previewData.vehicle_extra} THB</span>
            </div>
          )}
          ...
          >
            <div>
              ...
              <div className="total-val">
                {/* 🎯 แสดง total_price ที่รวม vehicle_extra มาแล้วจาก Backend */}
                {previewData?.total_price || 0}{" "}

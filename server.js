...
app.post("/users/register",async(req,res) => { //ใช้สำหรับลงทะเบียนผู้ใช้ใหม่ โดยรับข้อมูลจาก client ผ่าน HTTP POST และบันทึกลงใน MongoDB
...
app.post("/users/login", async (req, res) => { //ใช้สำหรับเข้าสู่ระบบผู้ใช้ โดยรับข้อมูลจาก client ผ่าน HTTP POST และตรวจสอบกับข้อมูลใน MongoDB
...
app.put("/users/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!fullname || !phone) {
      return res.status(400).json({ message: "please fill fullname and phone" });
    }
...
app.post("/vehicles", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!vehicle_type_id || !license_plate || !brand || !model || !color) {
      return res.status(400).json({ message: "please fill all fields except note" });
    }

    // หา vehicle_id ล่าสุด
    const lastVehicle = await db.find()
      .sort({ vehicle_id: -1 })
      .limit(1)
      .toArray();
